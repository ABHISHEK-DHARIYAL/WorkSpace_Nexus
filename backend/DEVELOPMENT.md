# WorkSpace Nexus Development & Execution Guide

---

This guide outlines how to run the WorkSpace Nexus application using either the **Unified Mode** (where the backend and frontend are consolidated and run together) or the **Separated Mode** (where frontend and backend run entirely independently as decoupled services).
...
> `WorkSpace Nexus API Backend is running separately on port 3001. Access the frontend dynamically for full workflow.`
...
- **Full Compilation**:
  ...
  _(Compiles the static frontend into `/dist` and bundles the Express backend code into a single, self-contained `/dist/server.cjs` file)._
